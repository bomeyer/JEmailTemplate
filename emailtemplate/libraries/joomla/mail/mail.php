...
class JMail extends PHPMailer
{
	...
	public function setBody($content)
	{
		...
		$emailTemplate = JPluginHelper::getPlugin('plg_emailtemplate', 'emailtemplate');
		$params = json_decode($emailTemplate->params);

		$this->Body = 
				($params->mailHeaderImage?'<img src="'.JURI::root().$params->mailHeaderImage.'" />':'').
				'<br/>'.chr(10).
				
				'<p>'.$this->replaceImageURL($params->mailHeaderText).'</p><br/>'.chr(10).
							
				JMailHelper::cleanText($this->replaceImageURL($content)).chr(10).
				
				'<br/>'.chr(10).
				'<p>'.$this->replaceImageURL($params->mailFooterText).'</p><br/>'.chr(10).
				
				($params->mailFooterImage?'<img src="'.JURI::root().$params->mailFooterImage.'" />':'');

		return $this;
	}

	public function replaceImageURL($txt) {
		return str_replace('<img src="images', '<img src="'.JURI::root().'images', $txt);
	}
